...
for word in anglicisms:
    ...
    if not word in word_scores:
        ...
        word_scores[word]['sca_mdcs_cosi'] = [
            sc.sca_mdcs(WWC = WWC_eng, word = word_eng, fns = words_eng, metric = 'cosine'),
            sc.sca_mdcs(WWC = WWC_ger, word = word_ger, fns = words_ger, metric = 'cosine')]
...
results = np.zeros( (len(anglicisms),len(scores)))
